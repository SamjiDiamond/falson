        ...
        try {
            $transaction = DB::transaction(function () use ($request, $user, $rac, $input, $ref, $logData) {
                ...
                Transaction::create([
                    ...
                    'extra' => $discount,
                    'commission' => $discount,
                    'category' => "data|".$rac->network."|".$rac->product_code,
                    'provider_price' => $rac->provider_price,
                    'profit' => isset($rac->provider_price) && $rac->provider_price !=0 ? $amountToDebit-$rac->provider_price : 0,
                    'paid_with' => 'wallet',
                    'date' => now(),
                    'ip_address' => $request->ip(),
                    'user_name' => $user->user_name,
