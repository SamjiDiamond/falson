    ...
    public function handle()
    {
        ...
        if ($s->service == "airtime") {
            ...
            switch (strtolower($server)) {
                case "7":
                    return $air->server7($r, $input['amount'], $input['phone'], $input['transid'], $input['network'], $input, $dada, "mcd");
                case "3":
        ...
        if ($s->service == "data") {
            ...
            switch (strtolower($rac->server)) {
                ...
                case "5":
                    return $air->server5($r, $input['coded'], $input['phone'], $input['transid'], $rac->network, $input, $dada, "mcd");
                case "4":
                    return $air->server4($r, $input['coded'], $input['phone'], $input['transid'], $rac->network, $input, $dada, "mcd");
                case "3":
        ...
        if ($s->service == "electricity") {
            ...
            switch (strtolower($rac->server)) {
                case "7":
                    return $air->server7($r, $input['network'], $input['phone'], $input['transid'], $input['network'], $input, $dada, "mcd");
                case "2":
